...
{#if syllabuses && syllabuses[0] && profile && profile.syllabus_ids}
  <div class="h-auto w-96 overflow-y-scroll flex flex-col">
    <input
      bind:value={search}
      class="input mb-3 rounded-sm"
    ...
    {#each syllabuses as s}
      {#if s.syllabus_name
        ...
        <div class="card p-2 mb-1 pb-6 rounded-sm">
          <header class="card-header flex justify-between items-baseline">
            <div class="">
              <h1 class="text-xl font-bold max-w-40">{s.syllabus_name}</h1>
              <div class="flex">
                <span class="badge variant-filled">{s.syllabus_code}</span>
                <span class="badge variant-filled">{s.syllabus_level}</span>
                <span class="badge variant-filled">{s.syllabus_board}</span>
              </div>
            </div>
            {#if s.status === false}
              <h1 class="badge variant-soft-warning">Coming Soon</h1>
            {:else}
              <button
